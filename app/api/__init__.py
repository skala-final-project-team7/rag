...
- main.py    FastAPI 앱 생성(create_app), 미들웨어, 헬스 체크(/healthz), lifespan
             (build_poc_deps + build_query_graph). uvicorn 진입점.
...
- deps.py    QueryGraphDeps 부트스트랩 (PoC: :memory: Qdrant + Fake + samples 자동 인덱싱)

이 계층은 요청 검증·응답 변환만 담당하고 비즈니스 로직은 app.pipeline / app.query에 둔다.

구현 상태:
- main.py    create_app + lifespan + /healthz   [feature11 통합 Phase 2]
- routes.py  POST /api/v1/rag/query SSE 라우트  [feature11 통합 Phase 2]
- errors.py  ErrorCode + ErrorResponse           [feature11 통합 Phase 2]
- deps.py    build_poc_deps                       [feature11 통합 Phase 2]
"""
