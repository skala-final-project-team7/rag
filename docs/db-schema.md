...
`file_size_bytes`, `download_url`, `local_path`(선택), `parent_page_id`, `last_modified`.

> `download_url`은 **사용자 노출용 URI/URL**(예: 운영은 Confluence 다운로드 URL, PoC는 file://
> URI)이며, 청커가 파일 시스템에서 직접 파일을 열 때는 `local_path`(있으면)를 우선 사용한다.
> 운영 어댑터는 `local_path`를 비워두고 별도 다운로드 헬퍼가 채운다. 결정 근거:
> `docs/adr/0001-attachment-source-url.md`.
...
- Qdrant Collection·Payload·인덱스, MongoDB 컬렉션, MySQL 테이블 변경 시 이 문서를 함께 수정한