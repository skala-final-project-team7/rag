...
→ **결정됨(ingestion↔rag 합의, `docs/adr/0003-ingestion-rag-shared-contracts.md` 항목 1):**
PoC ACL Pre-filtering은 **(a) `space_key` 기반**으로 확정한다. 수집 시 `allowed_groups`를
`["space:{space_key}"]`로 합성하고(`synthesize_space_acl`/`_synthesize_acl`), 검색 시
`app/query/acl.py:build_acl_filter`가 JWT `groups`(`space:{key}` 형식 — ADR 0002)에 OR 매칭한다.
(b) content restrictions API(페이지별 `allowed_groups`/`allowed_users`)는 명세 외 작업이라 보류이며,
도입 시 별도 ADR로 다룬다. `docs/db-schema.md` §1.4·`docs/ai/current-plan.md` 참조.
