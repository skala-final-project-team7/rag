...
- 상태: 채택 (항목 1·2·5는 PoC 확정 / 항목 3·4는 결정안 채택 + **적용 전 사람 승인 필요**)
- 날짜: 2026-05-26
- 작성자: 최태성
...
| 항목 | 제안 | 영향 | 적용 조건 |
|---|---|---|---|
| 3. `IngestionStage`에 `CRAWL` 추가 | enum 값 1개 additive 추가 + crawl 잡 기록 배선 | 양 레포 enum 동시 갱신/배포 필요(미갱신 읽기 측 `ValueError` 위험) | 승인 후 별도 change-set |
| 4. `soft_delete` 도입 | payload `is_deleted` 필드 + rag 검색 `must_not` 필터 | 전체 재색인 + 양 레포 동시 배포 | 승인 후 별도 change-set, Trash/Webhook 입력 경로 선행 |

항목 1·2·5는 문서/거버넌스 정합으로 본 change-set에서 처리하며 런타임 변경이 없다.
